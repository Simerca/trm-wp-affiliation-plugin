				...
				foreach($cashbacks as $cashback){
					...
					$args = array(
						...
						'meta_query' => array(
							...
							array(
								...
								array(
									'key' => 'utilisateur_affilie',
									'value' => $affiliateur->ID,
									'compare' => 'IN',
								)
							),array(
								'key' => 'cashback',
					...
					if($campagnes){
						...
						foreach($campagnes as $campagne){
							...
							var_dump($meta);
							echo "</pre>";
							if(in_array($affiliateur->ID, $meta['utilisateur_affilie'])){	

								if(!in_array($campagne->id, $campagnesAffiliation)) {
									$campagnesAffiliation[] = $campagne->ID;
								}
								
							}else{

								if(!in_array(304911, $campagnesAffiliation)) {
									$campagnesAffiliation[] = 304911;
								}
